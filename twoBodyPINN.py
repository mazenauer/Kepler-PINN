...
class NeuralNet(nn.Module):
    ...
    Methods are "forward" for evaluation, "xavier" for weight initialization, "lossFunction" and "fit" are self-explanatory. 
    """
    def __init__(self,input_dimension,output_dimension,n_hidden_layers,neurons,activation_function=nn.Tanh(),initial=torch.tensor([1.,0,0,1]),seed=0):
        ...
        self.size = np.sum([np.prod([i for i in el.shape]) for el in self.parameters()])
    ...
    def conservedQuantities(self,qx,px,qy,py):
        d = (qx.square()+qy.square()).sqrt()#+1e-5
    ...
    def lossFunction(self,t_train,weights=[1,1,1,0],batch_size=None):
        """
        Perhaps the most important piece of code. Takes the sample training times as a pytorch array, needs property "requires_grad=True".
        The weights give a different weight to the three kinds of losses: ODE loss, initial conditions loss, and loss from conservation laws.
        Returns weighted sum of losses as a real number of type pytorch.tensor. If "batch_size" is given then stochastic gradient descent 
        is used.
        """
        ...
        d3 = (qx_pred.square()+qy_pred.square()).pow(1.5)#+1e-7
        ...
        #Loss from conserved quantities
        H,L,Ax,Ay = self.conservedQuantities(*u_pred.T)
        H = 0.5*(qx_dot.square()+qy_dot.square())-1/((qx_pred.square()+qy_pred.square()).sqrt()+1e-5)
        CLloss = (H-H0).square().mean() + (L-L0).square().mean() + (Ax-Ax0).square().mean() + (Ay-Ay0).square().mean()
        """
        ...
        #No longer required: supervised loss from exact solutions
#        EXloss = 0
#        if exacts != None:
#            sx,spx,sy,spy = exacts
#            EXloss = (qx_pred-torch.tensor(sx)).square().mean() + (px_pred-torch.tensor(spx)).square().mean() + (qy_pred-torch.tensor(sy)).square().mean() + (py_pred-torch.tensor(spy)).square().mean()
        
        return (weights[0]*DEloss+weights[1]*ICloss+weights[2]*CLloss+weights[3]*Regloss)/sum(weights)
        
    def validationLoss(self,xexact,yexact,t_train):
        u_pred = self(t_train).detach().numpy()
        qx_pred = u_pred[:,0]
        qy_pred = u_pred[:,2]
        return L2Error(xexact,yexact,qx_pred,qy_pred)
    ...
    def fit(self,t_train,epochs,verbose=False,weights=[1,1,1,0],optimizer=None,
        ...
        for i in range(epochs):
            ...
            validationLoss.append(self.validationLoss(xexact,yexact,t_train))
            if loss>maxtol:
                flag = "maxtol"
                return history,validationLoss,flag
                #raise Exception("Loss too large")
    ...
    def xavier(self,seed=None):
        """
        Xavier initialization of weights and biases, details not important, code copied from lecture DLSC.
        """
        if seed==None :
            seed = self.seed
...
def goodnessOfFit(x,y,E,L):
    ...
    sqrError = (r-rfunc(phi,alpha))**2
    return sqrError,np.sqrt(np.mean(sqrError))
"""

def goodnessOfFit(xpred,ypred,xexact,yexact,T):
    err = ((xexact-xpred)**2+(yexact-ypred)**2)/(xexact**2+yexact**2)
    L2 = 0.5*T*np.mean(err[1:]+err[:-1])
    return L2